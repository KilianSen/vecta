...
/**
 * Wrapper mode: {@code java -jar anymcp.jar [server.jar] [server args]} starts anymcp and then the
...
public final class Launcher {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        if (jar == null) {
            fail("no server jar found. Run java -jar anymcp.jar <server.jar> [args], or set serverJar in " + cfg.file);
        }
        ...
        if (mainClass == null) fail(jar.getName() + " has no Main-Class; start it with -javaagent:anymcp.jar instead");

        addToClassPath(jar);
        String cp = System.getProperty("java.class.path", "");
        System.setProperty("java.class.path", jar.getPath() + (cp.isEmpty() ? "" : File.pathSeparator + cp));

        Anymcp.start(cfg, "wrapper");
    ...
    static File detectServerJar(File dir) {
        ...
        for (File f : files) {
            ...
            if (!f.isFile() || !n.endsWith(".jar") || n.contains("installer") || n.startsWith("anymcp")) continue;
    ...
    private static void addToClassPath(File jar) throws Exception {
        Instrumentation inst = Anymcp.instrumentation();
        if (inst != null) {
        ...
        fail("cannot add " + jar.getName() + " to the class path; start with java -javaagent:anymcp.jar -jar " + jar.getName());
    }

    private static void fail(String msg) {
        System.err.println("[anymcp] " + msg);
        System.exit(2);
    }
}
