...
final class GuardCodec {
    ...
    static byte[] key(String secret) {
        try {
            return MessageDigest.getInstance("SHA-256").digest(("anymcp-guard-key-v1:" + secret).getBytes(StandardCharsets.UTF_8));
    ...
    private static byte[] mac(byte[] key, int verCmd, int fam, byte[] addr, long ts, byte[] nonce)
        ...
        m.update("anymcp-guard-v1".getBytes(StandardCharsets.US_ASCII));
